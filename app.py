...
# 🌟 核心修复：纯 CSS Checkbox 模态框，彻底绕过 Streamlit 路由拦截，告别灰屏！
st.markdown("""
<style>
        ...
        try:
            ...
            if q_res.get("code") == 0:
                ...
                if status == "succeeded":
                    ...
                    # 🌟 使用 Checkbox Hack 渲染图片，绝不触发路由拦截！
                    imgs_html = ""
                    for idx, url in enumerate(urls):
                        m_id = f"cb_p_{safe_task_id}_{idx}"
                        imgs_html += f'<label for="{m_id}"><img src="{url}" class="result-thumb" style="border: 2px solid #00ff88; margin-top:10px;"></label><input type="checkbox" id="{m_id}" class="modal-checkbox"><label for="{m_id}" class="img-modal-overlay"><img src="{url}"></label>'
    ...
    with st.container(height=700):
        for item in reversed(tasks_list):
            ...
            if item['status'] == 'running':
                ...
                safe_item_id = str(item['task_id']).replace("-", "")
                for idx, url in enumerate(item['urls']):
                    m_id = f"cb_h_{safe_item_id}_{idx}"
                    # 🌟 同样的 Checkbox 渲染法，绝不白屏！
                    st.markdown(f'<label for="{m_id}"><img src="{url}" class="result-thumb"></label><input type="checkbox" id="{m_id}" class="modal-checkbox"><label for="{m_id}" class="img-modal-overlay"><img src="{url}"></label>', unsafe_allow_html=True)
